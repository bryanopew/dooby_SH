...
import {Col, Row, TextMain, TextSub} from '~/styles/stylesHS/styledConsts';
...
const FoodList = ({item}) => {
  console.log('FoodList : item:', item.item.mainAttUrl);
  console.log('FoodList : item:', `${BASE_URL}${item.item.mainAttUrl}`);
  return (
    <Container>
      <Row>
        <Thumbnail
        ...
        <AddToCartBtn onPress={() => console.log('AddToCartBtn Clicked')}>
          <AddToCartBtnImage
            source={require(`~/Assets/Images/24_foodAdd.png`)}
          />
        </AddToCartBtn>
      </Row>
